  ...
  return (
    <div className="flex h-full">
      <div className="w-[420px] shrink-0 border-r border-[var(--color-border-faint)] bg-[var(--color-bg-base)] flex flex-col">
        <div className="flex items-center justify-between gap-2 px-3 h-11 border-b border-[var(--color-border-faint)] bg-[var(--color-bg-base)] shrink-0">
          <div className="flex items-center gap-2 min-w-0">
            <span className="text-[14px] font-medium text-[var(--color-text-secondary)] shrink-0">Playground</span>
            <span className="text-[11px] px-1.5 py-0.5 rounded bg-[var(--color-border-faint)] text-[var(--color-text-tertiary)] uppercase tracking-wider shrink-0">Agent</span>
          </div>
          <div className="flex items-center gap-1.5 shrink-0 min-w-0">
            <AgentModelPicker value={currentAgentModel} onChange={setPlaygroundAgentModel} />
            <button
              onClick={handleReset}
              className="text-[12px] text-[var(--color-text-tertiary)] hover:text-[var(--color-text-primary)] transition-colors px-1.5 py-1 rounded focus-visible:outline focus-visible:outline-1 focus-visible:outline-[var(--color-accent)]"
              title="Clear conversation"
            >
              Clear
            </button>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex items-center gap-2 px-3 h-11 border-b border-[var(--color-border-faint)] bg-[var(--color-bg-base)] shrink-0">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            {linkedWorkflow ? (
              <div className="flex items-center gap-1 min-w-0 px-2 py-0.5 rounded bg-[var(--color-border-faint)] border border-[var(--color-border-faint)]">
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="text-[var(--color-text-quaternary)] shrink-0"><path d="M12 20h9M16.5 3.5a2.121 2.121 0 113 3L7 19l-4 1 1-4 12.5-12.5z" /></svg>
                <span className="text-[12.5px] text-[var(--color-text-secondary)] truncate max-w-[180px]" title={linkedWorkflow.name}>{linkedWorkflow.name}</span>
                <button onClick={handleUnlink} className="text-[var(--color-text-quaternary)] hover:text-[var(--color-text-secondary)] transition-colors p-0.5 -mr-1 shrink-0" title="Unlink">
              ...
              <select
                ...
                className="bg-[var(--color-border-faint)] border border-[var(--color-border-faint)] rounded px-2 py-1 text-[12px] text-[var(--color-text-tertiary)] outline-none hover:border-[var(--color-border-soft)] max-w-[200px]"
              >
            ...
            <span className="text-[12px] text-[var(--color-text-quaternary)] font-mono shrink-0">{draft.nodes.length}n · {draft.edges.length}e</span>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <button
              ...
              className="text-[var(--color-text-quaternary)] hover:text-[var(--color-text-secondary)] transition-colors p-1.5 disabled:opacity-30 disabled:cursor-not-allowed"
              title="Reset canvas"
            >
            ...
            <button
              ...
              className={cn(
                ...
                canRun ? 'bg-green-500/20 text-green-300 border border-green-500/30 hover:bg-green-500/30' : 'bg-[var(--color-border-faint)] text-[var(--color-text-quaternary)] border border-[var(--color-border-faint)] cursor-not-allowed',
              )}
            ...
            <button
              ...
              disabled={!canExport || isRunning}
              className={cn(
                'text-[13px] px-2.5 py-1 rounded-md border border-[var(--color-border-soft)] transition-colors',
                canExport && !isRunning ? 'text-[var(--color-text-secondary)] hover:border-[var(--color-border-strong)] hover:text-[var(--color-text-primary)]' : 'text-[var(--color-text-quaternary)] opacity-40 cursor-not-allowed',
              )}
            ...
            <button
              ...
              className={cn(
                ...
                canExport && !isRunning
                  ...
                  : 'bg-[var(--color-border-faint)] text-[var(--color-text-quaternary)] cursor-not-allowed',
              )}
