  ...
  return (
    <div ref={ref} className="relative w-56 shrink-0">
      <button
        ...
        className={cn(
          'w-full flex items-center justify-between gap-2 bg-[var(--color-border-faint)] border border-[var(--color-border-soft)] rounded-md px-2.5 py-1.5 text-[13px] hover:border-[var(--color-border-strong)] transition-colors text-left',
      ...
      {open && (
        <div className="absolute top-full right-0 mt-1 w-[340px] z-50 bg-[var(--color-bg-overlay)] border border-[var(--color-border-soft)] rounded-lg shadow-[var(--color-surface-shadow)] animate-scale-in overflow-hidden">
          <div className="px-2 py-2 border-b border-[var(--color-border-faint)] flex items-center gap-2">
            <input
              autoFocus
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search models…"
              className="flex-1 bg-[var(--color-border-faint)] border border-[var(--color-border-faint)] rounded px-2 py-1 text-[13px] text-[var(--color-text-primary)] outline-none focus:border-[var(--color-border-strong)] placeholder:text-[var(--color-text-quaternary)]"
            />
            <button
              onClick={() => setShowAll(!showAll)}
              className={cn(
                'text-[11px] px-1.5 py-1 rounded border transition-colors',
                showAll ? 'bg-[var(--color-border-faint)] border-[var(--color-border-strong)] text-[var(--color-text-primary)]' : 'border-[var(--color-border-soft)] text-[var(--color-text-tertiary)] hover:text-[var(--color-text-secondary)]',
              )}
          ...
          <div className="max-h-[380px] overflow-y-auto py-1">
            {grouped.length === 0 && (
              <div className="px-3 py-6 text-center text-[13px] text-[var(--color-text-quaternary)]">No matches</div>
            )}
            {grouped.map((group) => (
              <div key={group.label}>
                <div className="px-3 pt-2 pb-1 text-[10.5px] uppercase tracking-[0.08em] text-[var(--color-text-quaternary)] font-medium">{group.label}</div>
                {group.items.map((m) => (
                  <button
                    key={m.id}
                    onClick={() => { onChange(m.id); setOpen(false); setSearch('') }}
                    className={cn(
                      'w-full text-left px-3 py-2 hover:bg-[var(--color-border-faint)] transition-colors',
                      m.id === value && 'bg-[var(--color-border-faint)]',
                    )}
                  >
                    <div className="flex items-center gap-1.5">
                      <span className="text-[13px] text-[var(--color-text-primary)] font-medium truncate flex-1">{m.name}</span>
                      {m.id === value && (
                    ...
                    <div className="text-[11px] text-[var(--color-text-quaternary)] mt-1 font-mono truncate">
...
const TONE: Record<string, string> = {
  ...
  slate: 'bg-[var(--color-border-faint)] text-[var(--color-text-secondary)]',
